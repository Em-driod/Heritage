...
const Hero: React.FC = () => {
  ...
  // It only needs 'initial', 'animate', and 'exit' states.
  // Staggering will be handled by the parent 'motion.div' with containerVariants.
  ...
  return (
    <div className="py-4 sm:py-8 md:py-12 lg:py-24 overflow-hidden"> {/* Add overflow-hidden to prevent horizontal scroll from initial animation */}
      <div
      ...
      >
        ...
        >
          ...
          <motion.div
            ...
            variants={containerVariants} // Apply container variants to this section
          >
            ...
            <motion.button
              ...
              variants={itemVariants} // Applies the same slide-up animation
              whileHover={{ scale: 1.08 }} // Button specific hover animation
              whileTap={{ scale: 0.95 }} // Button specific tap animation
            >
          ...
          <motion.div
            ...
            variants={cardVariants} // Card specific animation (scale-in)
          >
            {/* Glow effects behind the card - can also be motion.div if you want them animated */}
            ...
            <div className="bg-white rounded-xl shadow-2xl p-0 sm:p-8">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-8">
                ...
                {['Yoruba', 'Igbo', 'Japan'].map((culture) => (
                  <motion.button
                    ...
                    whileHover={{ scale: 1.05 }} // Slight scale on hover for buttons
                    whileTap={{ scale: 0.95 }} // Slight scale on tap for buttons
                    variants={itemVariants} // Apply item variants to buttons as well
                  >
                    {/* Image as the background with proper object-cover */}
                    <img src={`/${culture.toLowerCase()}.png`} alt={`${culture} Culture`} className="absolute inset-0 w-full h-full object-cover rounded-md" />

                    {/* Overlay for readability of text, changes on hover/selected */}
              ...
              <div className="flex flex-col items-center mb-6">
                <motion.svg
                  ...
                  transition={{ repeat: Infinity, repeatType: "reverse", duration: 1.5 }} // Subtle pulsating arrow
                >
              ...
              {/* Use AnimatePresence for exit animations on list items */}
              <AnimatePresence mode="wait"> {/* 'wait' mode ensures new items don't animate in until old ones are out */}
                {isLoading ? (
                  ...
                  <motion.div
                    key="assets-container" // Unique key for AnimatePresence to track this entire block
                    variants={containerVariants} // Re-use container variants for stagger effect
                    initial="hidden" // Ensure the container has initial state
                    animate="visible" // Ensure the container animates in
                    className="space-y-4 mb-8"
                  >
                    {generatedAssets.map((asset) => (
                      <motion.div
                        key={asset.label}
                        variants={assetItemVariants} // Apply slide-in/out for each asset
                        // No need for initial="hidden" animate="visible" here as parent's containerVariants will manage it,
                        // unless you specifically want each item to manage its own entry regardless of stagger.
                        // However, for clean AnimatePresence, defining the 'variants' prop is enough for it to pick up 'initial', 'animate', 'exit' from them.
                      >
                        ...
                        {asset.label === 'Amala' && selectedCulture === 'Yoruba' && showAmalaSpecificLoading && (
                          <AnimatePresence> {/* AnimatePresence for AmalaSpecificLoader */}
                              <AmalaSpecificLoader />
                          </AnimatePresence>
                        )}
                      </motion.div>
              ...
              <div className="flex flex-col sm:flex-row items-center justify-between space-y-4 sm:space-y-0">
                <div className="flex space-x-2">
                  {/* These buttons can also have whileHover/whileTap */}
                  <motion.button
