...
const Hero: React.FC = () => {
  ...
  const cultureButtonsRef = useRef<(HTMLButtonElement | null)[]>([]);
  const generatedAssetsRef = useRef<(HTMLDivElement | null)[]>([]);
  ...
  useEffect(() => {
    ...
    // Ensure elements start invisible or off-set before animating
    gsap.set([leftSectionRef.current, rightSectionRef.current], { opacity: 0 });
    gsap.set(cultureButtonsRef.current, { opacity: 0, y: 20 });

    // Animate the main hero background (if desired, though usually handled by CSS background-image)
    tl.from(heroRef.current, { opacity: 0, duration: 1.2 }, "start");

    // Animate left section (text and button)
    tl.from(leftSectionRef.current, { x: -100, opacity: 0, duration: 1, stagger: 0.2 }, "start+=0.3");

    // Animate right section (card)
    tl.from(rightSectionRef.current, { y: 100, opacity: 0, duration: 1 }, "start+=0.5");

    // Animate culture buttons after the card appears
    tl.from(cultureButtonsRef.current, {
      opacity: 0,
      y: 20,
      stagger: 0.1,
      duration: 0.6,
      ease: "back.out(1.7)",
    }, "start+=1.2"); // Adjusted timing to follow the card animation

  }, []); // Empty dependency array ensures this runs only once on mount

  // Animation for generated assets when they load/change
  useEffect(() => {
    if (!isLoading && generatedAssets.length > 0) {
      // Ensure existing assets are hidden before animating new ones
      gsap.set(generatedAssetsRef.current, { opacity: 0, x: 20 });
      gsap.from(generatedAssetsRef.current, {
        opacity: 0,
        x: 20,
        stagger: 0.1,
        duration: 0.5,
    ...
    <div className="py-4 sm:py-8 md:py-12 lg:py-24">
      ...
      >
        ...
        <div className="relative z-10 container mx-auto flex flex-col lg:flex-row items-center lg:items-start justify-center gap-8 lg:gap-12 max-w-7xl">
          {/* Left Section */}
          <div ref={leftSectionRef} className="flex flex-col items-center mt-6 lg:items-start text-center lg:text-left max-w-full lg:max-w-xl px-4 sm:px-0">
          ...
          <div ref={rightSectionRef} className="relative w-full max-w-md lg:max-w-lg lg:mt-0">
            ...
            <div className="bg-white rounded-xl shadow-2xl p-0 sm:p-8">
              ...
              <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-8">
                {/* Culture Buttons */}
                {['Yoruba', 'Igbo', 'Japan'].map((culture, index) => (
                  <button
                    key={culture}
                    ref={el => cultureButtonsRef.current[index] = el} // Assign ref to each button
                ...
                <div className="space-y-4 mb-8">
                  {generatedAssets.map((asset, index) => (
                    <React.Fragment key={asset.label}>
                      <div ref={el => generatedAssetsRef.current[index] = el} className="flex items-center justify-between">
                        <div className="flex items-center space-x-3">
