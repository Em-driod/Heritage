...
const Hero: React.FC = () => {
  ...
  const AmalaSpecificLoader: React.FC = () => (
    <div className="flex flex-col justify-center bg-gray-50 p-4 rounded-lg border border-gray-200 text-sm text-gray-800 space-y-4">
      ...
      <p><span className="font-semibold text-red-500">Regions Known For It:</span> Deeply rooted in Oyo, Osun, Ibadan. Especially beloved in Ibadan.</p>
  ...
  return (
    <div className="py-4 sm:py-8 md:py-12 lg:py-24">
      ...
      >
        ...
        <div className="relative z-10 container mx-auto flex flex-col lg:flex-row items-stretch justify-center gap-8 lg:gap-12 max-w-7xl min-h-screen">
          {/* Left Section */}
          <div className="flex flex-col justify-center items-center lg:items-start text-center lg:text-left max-w-full lg:max-w-xl px-4 sm:px-0 h-full py-10">
...
<div className="relative w-full max-w-md lg:max-w-lg h-full">
  {/* Glow Effects */}
  <div className="absolute -right-10 top-1/2 transform -translate-y-1/2 w-48 h-48 bg-purple-500 opacity-40 blur-3xl z-0"></div>
  <div className="absolute top-0 left-1/2 transform -translate-x-1/2 w-32 h-16 bg-green-400 opacity-30 blur-2xl z-0"></div>
  <div className="absolute -left-10 bottom-1/2 transform translate-y-1/2 w-36 h-24 bg-yellow-300 opacity-25 blur-2xl z-0"></div>
  <div className=' bg-white shadow-blue-500'>
            ...
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 mb-8">
              {['Yoruba', 'Igbo', 'Japanese'].map((culture) => (
                <button
                key={culture}
                className={`relative flex items-center justify-center p-3 rounded-md text-sm font-medium text-white h-20 overflow-hidden ${
