...
const Waitlist: React.FC = () => {
  ...
  return (
    <div className="min-h-screen mt-12 bg-white flex flex-col md:flex-row items-center md:justify-center px-4 sm:px-6 md:px-12 py-8 sm:py-12 text-gray-900">
      {/* Left Section */}
      <motion.div className="w-full max-w-xl space-y-4 sm:space-y-6 md:mr-8 mt-8 sm:mt-14" variants={containerVariants} initial="hidden" animate="visible">
        ...
        <motion.h2 className="text-2xl sm:text-3xl font-bold text-black mb-4 sm:mb-7" variants={itemVariants}>Free in Beta!</motion.h2>
        ...
        <motion.div className="flex w-full max-w-md mt-6 sm:mt-8 gap-2" variants={itemVariants}>
            ...
            {generatedAssets.map((item) => (
              <motion.li key={item.id} className="flex items-center justify-between bg-white rounded-lg p-2 sm:p-3 relative group" variants={itemVariants}>
                <div className="flex items-center relative">
                  ...
                  <span className="font-medium text-gray-800 text-sm sm:text-sm">{item.name}</span>
                </div>
